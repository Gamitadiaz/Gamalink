...
import type { Config, Plan, PlanModalMode } from '@/lib/types';

type Props = {
  config: Config;
  configDraft: Config;
  setConfigDraft: (c: Config) => void;
  configLoading: boolean;
  configSaved: boolean;
  guardarConfig: () => Promise<void>;
  planes: Plan[];
  setPlanSeleccionado: (p: Partial<Plan>) => void;
  setPlanModalMode: (m: PlanModalMode) => void;
  togglePlanActivo: (p: Plan) => Promise<void>;
  eliminarPlan: (id: number) => Promise<void>;
};

export default function ConfiguracionView({
  config, configDraft, setConfigDraft, configLoading, configSaved, guardarConfig,
  planes, setPlanSeleccionado, setPlanModalMode, togglePlanActivo, eliminarPlan,
}: Props) {
  return (
