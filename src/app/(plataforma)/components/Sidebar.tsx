...
const navItems = [
  ...
  { id: 'clientes',      label: 'Clientes'      }, // Activar solo si es membresia
  { id: 'reportes',      label: 'Reportes'      }, 
  { id: 'inventario', label: 'Inventario' },
  { id: 'configuracion', label: 'Configuración' }, //agregar gestion de usuarios y permisos 
] as const;
  ...
  return (
    ...
    >
      ...
      <div className="p-5" style={{ borderBottom: '1px solid var(--border)' }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] t-sidebar-muted mt-0.5 truncate max-w-[130px]">{nombreNegocio}</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 p-3 space-y-1">
        {navItems.map(item => {
          // Evaluamos si la ruta actual coincide con el item del menú
          ...
          return (
            <Link 
              key={item.id} 
              href={`/${item.id}`}
              onClick={onCloseSidebar} // Cierra el menú en móviles al hacer clic
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold transition ${isActive ? 't-nav-active' : 't-sidebar-nav-inactive'}`}
            >
              ...
              {item.id === 'clientes' && (
                <span className="ml-auto text-xs px-2 py-0.5 rounded-full font-black"
                  style={{
                    backgroundColor: isActive ? 'rgba(255,255,255,0.2)' : 'color-mix(in srgb, var(--primary) 20%, transparent)',
